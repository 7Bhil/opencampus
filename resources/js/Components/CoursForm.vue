<!-- resources/js/Components/CoursForm.vue -->
<template>
    <form @submit.prevent="submit">
        ...
        <div class="space-y-6">
            ...
            <!-- Visibilité (uniquement pour les profs) -->
            <div v-if="userType === 'Professeur'">
                <div class="flex items-center">
                    <input
                        id="est_public"
...
const props = defineProps({
    userType: {
        type: String,
        required: true,
        validator: (value) => ['Professeur', 'Etudiant'].includes(value)
    },
    submitRoute: {
        type: String,
        required: true
...
const form = useForm({
    ...
    prix: null,
    fichier: null,
    est_public: props.userType === 'Professeur', // Profs: public par défaut
})

// Texte du bouton selon le type d'utilisateur
const submitText = computed(() => {
    if (props.userType === 'Etudiant') {
        return 'Soumettre pour modération'
    }
...
const submit = () => {
    form.post(props.submitRoute, {
        onSuccess: () => {
            form.reset()
            router.visit(props.successRoute)
        }
    })
}
</script>
